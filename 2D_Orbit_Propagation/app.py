...
st.write("Enter the Initial Radius and Velocity Vector to Find your Satellite's Orbit around Earth")
...
with st.form("Propagation_Input"):
    ...
    with colr:
        ...
        r0_x = st.number_input("X Component (km)", value=7000.0, step=0.01)
        r0_y = st.number_input("Y Component (km)", value=0.0, step=0.01)

    with colv:
        st.latex(r"\text{Velocity Vector}")
        v0_x = st.number_input("X Component (km/s)", value=0.0000, step=0.0001, format="%.4f")
        v0_y = st.number_input("Y Component (km/s)", value=7.5000, step=0.0001, format="%.4f")

    with colt:
        st.latex(r"\text{Simulation Parameters}")
        colt1, colt2 = st.columns([3, 2])
        with colt1:
            tf_val = st.number_input("Simulation Time", min_value=1.0, value=1.0, step=0.01)
